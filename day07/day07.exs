defmodule Util do
  def contained_bags(rules, node) do
  ...
  def outermost_color(rules, acc, node) do
...
|> Util.outermost_color(MapSet.new(), "shiny gold")
|> MapSet.size()
|> IO.inspect()

Util.contained_bags(contains, "shiny gold") |> IO.inspect()
